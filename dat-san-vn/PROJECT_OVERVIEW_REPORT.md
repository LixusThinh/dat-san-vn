...
| **BullMQ & Redis** | `apps/api/queues` | Queue / Jobs | Quản lý timer 15 phút auto-cancel booking rất mượt mà. |
...
* **`Queue` (Booking Expiration)**:
  - Dùng BullMQ, khi tạo booking thành công sẽ đẩy 1 job delay 15 phút. Sau 15p nếu status vẫn `PENDING` sẽ tự chuyển `CANCELLED` và trả slot về `AVAILABLE`.
...
5. Đẩy job vào BullMQ. User có 15 phút để chủ sân Confirm hoặc Thanh toán.
6. Hết 15 phút, Queue xử lý, nếu vẫn PENDING -> `CANCELLED`, slot về `AVAILABLE`.
